...
>
> **Strategy.** $\mathrm{A1}$ is not just ordinary perbromate; from Q3 it is radioactive $\mathrm{^{83}BrO_{4}^{-}}$. The second first-order constant in the scheme, $k_{2}=0.0048\ \mathrm{min^{-1}}$, gives
>
> $$t_{1/2}=\frac{\ln 2}{k_{2}}=\frac{0.693}{0.0048}=144\ \mathrm{min}=2.40\ \mathrm{h},$$
>
> which matches the half-life of $\mathrm{^{83}Br}$ beta decay to $\mathrm{^{83}Kr}$. Therefore the "spontaneous decomposition" of **A1** is best understood as the nuclear decay of the bromine atom in the perbromate ion, followed by loss of the oxygen ligand shell as dioxygen:
>
> $$\boxed{\mathrm{^{83}BrO_{4}^{-}\longrightarrow{}^{83}Kr+2\,O_{2}+\beta^{-}+\bar{\nu}_{e}}}$$
>
> The beta particle accounts for charge balance; the chemical gaseous elemental substances are $\mathrm{^{83}Kr}$ and $\mathrm{O_{2}}$.
>
> | Species | Formula | Notes |
> |---|---|---|
> | **C** | $\mathrm{^{83}Kr}$ | monatomic krypton formed by $\beta^{-}$ decay of $\mathrm{^{83}Br}$ |
> | **D** | $\mathrm{O_{2}}$ | dioxygen released from the four oxygen atoms of perbromate |
>
> Thus $n(\mathrm{C}):n(\mathrm{D})=n(\mathrm{Kr}):n(\mathrm{O_{2}})=1:2$, exactly matching the problem statement. This also explains why the process is first order: it is governed by radioactive decay rather than by an ordinary perbromate disproportionation.
>
> **Reference note.** The reported half-life for $\mathrm{^{83}Br\to{}^{83}Kr}$ is about $2.39\ \mathrm{h}$, consistent with the scheme value. See, for example, *Decay of Br-83*, *Nuclear Physics* **45** (1963), 336-340.
>
...
> The step $\mathrm{A1\to C+D}$ is the $\beta^{-}$ decay of $\mathrm{^{83}Br}$ in $\mathrm{^{83}BrO_4^-}$; the value shown in the scheme image, $k_{2}=4.8\times10^{-3}\ \mathrm{min^{-1}}$, gives $t_{1/2}=144\ \mathrm{min}$, matching $\mathrm{^{83}Br}$. Thus
...
>
> **Strategy.** "Exactly the same unusual method like $\mathrm{B\to A1}$" again means beta decay inside a pre-organised ligand shell. To obtain a xenon binary compound that is **not a fluoride**, the strongest candidate is a chloride ligand cage around radioactive iodine:
>
> $$\mathrm{K[{}^{129}ICl_{4}] \xrightarrow{\beta^-} {}^{129}XeCl_{4}+\ K^{+}+\beta^{-}+\bar{\nu}_{e}}.$$
>
> In $\mathrm{[ICl_4]^-}$, iodine is formally +III. After $\beta^{-}$ decay, the nuclear charge increases by one, so retaining the four chloride ligands gives neutral xenon(IV) tetrachloride, $\mathrm{XeCl_4}$. This is directly analogous to the hot-atom inheritance in $\mathrm{^{83}SeO_4^{2-}\to{}^{83}BrO_4^-}$.
>
> **Pinning down F from $w_{\mathrm{K}}=12.62\%$.**
>
> Using $\mathrm{^{129}I}$, the formula mass of $\mathrm{K[{}^{129}ICl_4]}$ is
>
> $$M=39.10+128.90+4(35.45)=309.8\ \mathrm{g\,mol^{-1}},$$
>
> so
>
> $$w_{\mathrm{K}}=\frac{39.10}{309.8}=0.1262=12.62\%.$$
>
> This exact match is much stronger than the previous periodate/fluoroperiodate fit and also gives a binary xenon compound containing no fluorine.
>
> **Answers.**
>
> | Symbol | Formula | Name / comment |
> |---|---|---|
> | **E** | $\boxed{\mathrm{{}^{129}XeCl_{4}}}$ | xenon tetrachloride, a binary Xe-Cl compound; square-planar Xe(IV) environment |
> | **F** | $\boxed{\mathrm{K[{}^{129}ICl_{4}]}}$ | potassium tetrachloroiodate(III), $w_{\mathrm{K}}=12.62\%$ |
>
> **Reference note.** This assignment is literature-backed: G. J. Perlow and M. R. Perlow reported Mossbauer evidence for $\mathrm{XeCl_4}$ formed from $\mathrm{K[{}^{129}ICl_4]}$; see "Mossbauer Effect Evidence for the Existence and Structure of XeCl4," *J. Chem. Phys.* **1964**, 41, 1157-1158, DOI: [10.1063/1.1726022](https://doi.org/10.1063/1.1726022).
>
...
- **C、D** 分别为 $\mathrm{^{83}Kr}$ 与 $\mathrm{O}_{2}$。关键证据不是普通化学配平，而是题图 $k_{2}=0.0048\ \mathrm{min}^{-1}$ 对应 $t_{1/2}=144\ \mathrm{min}\approx2.40\ \mathrm{h}$，正好匹配 $\mathrm{^{83}Br}\to\mathrm{^{83}Kr}$ 的 $\beta^{-}$ 衰变半衰期；因此
  $$\mathrm{^{83}BrO_{4}^{-}\rightarrow{}^{83}Kr+2O_{2}+\beta^{-}+\bar{\nu}_{e}}.$$
- **E** 是 $\mathrm{^{129}XeCl}_{4}$——由 $\mathrm{K[{}^{129}ICl_{4}]}$ 中的 $\mathrm{^{129}I}$ 衰变而来，女核继承四个氯配体，形成中性的氙(IV)二元氯化物；
- **F** 为 $\mathrm{K[{}^{129}ICl_{4}]}$（钾四氯合碘(III)酸盐），$w_{\mathrm{K}}=39.10/(39.10+128.90+4\times35.45)=12.62\%$，与题设精确吻合。

**三处容易误判之处。** 此题有三处特别容易被常规无机直觉带偏的地方；修正后只有第一处仍是真正的 caveat，后两处应按核衰变与文献线索处理：
...
2. **C:D = 1:2 不应硬凑成 $\mathrm{O}_{2}/\mathrm{O}_{3}$**——若只做普通化学配平，确实容易被"两种气态单质"诱导到 $\mathrm{O}_{2}$ 与 $\mathrm{O}_{3}$。但题图给出的 $k_{2}=0.0048\ \mathrm{min}^{-1}$ 对应半衰期约 2.40 h，几乎直接锁定 $\mathrm{^{83}Br}$ 的 $\beta^{-}$ 衰变。因此正确理解是 $\mathrm{^{83}BrO}_{4}^{-}$ 核衰变为 $\mathrm{^{83}Kr}$，四个氧原子释放为 $2\,\mathrm{O}_{2}$，比例自然为 $\mathrm{Kr}:\mathrm{O}_{2}=1:2$；
3. **F 的 12.62% 精确锁定 $\mathrm{K[{}^{129}ICl_{4}]}$**——$\mathrm{K[{}^{129}ICl_{4}]}$ 的钾质量分数为 $12.62\%$，且文献中正是通过 $\mathrm{K[{}^{129}ICl_{4}]}$ 的 $\beta^{-}$ 衰变证明了 $\mathrm{XeCl}_{4}$ 的存在和结构。相比之下，$\mathrm{K}[\mathrm{IO}_{2}\mathrm{F}_{4}]\cdot2\mathrm{H}_{2}\mathrm{O}$ 虽然质量分数也接近，但会导向含氧/含氟配位壳层，与题目"不含氟的氙二元化合物"和已知 $\mathrm{XeCl}_{4}$ 文献线索都不如 $\mathrm{KICl}_{4}$ 干净。

**Bateman 动力学收官。** Q6 是经典"两步一级串联反应 + 中间体浓度极值"题，套用 Bateman 公式
$$t_{\max}=\frac{\ln(k_{2}/k_{1})}{k_{2}-k_{1}}$$
代入题图中的 $k_{1}=0.031\ \mathrm{min}^{-1}$、$k_{2}=0.0048\ \mathrm{min}^{-1}$ 即得 $t_{\max}\approx 71\ \mathrm{min}$。这一步只要公式记得住就是送分题，但需留意**不要误读 $k_{2}$ 的小数点**——题图中是 $0.0048$，不是 $0.048$；而且 $k_{2}$ 的半衰期恰好反过来支持 Q5 的 $\mathrm{^{83}Br}$ 核衰变解释。
...
2. **配位壳层继承 = 热原子化学的核心**——$\beta^{-}$ 衰变只改核内质子数，母体的配位壳层可在极短时间内限制女核的初始化学环境。$\mathrm{SeO}_{4}^{2-}\to\mathrm{BrO}_{4}^{-}$ 是氧配位壳层的继承；$\mathrm{[ICl}_{4}]^{-}\to\mathrm{XeCl}_{4}$ 是氯配位壳层的继承；
3. **遇到速率常数要想到核半衰期**——本题 $k_{1}$ 与 $k_{2}$ 分别对应 $\mathrm{^{83}Se}$ 和 $\mathrm{^{83}Br}$ 的核衰变，不能把全部箭头都当成普通化学反应处理。B 的质量比仍需作为 caveat 交代，但 C/D 与 E/F 现在有更强的核化学解释；
...
**难度评级：★★★★★**——超纲核化学背景 + 同位素半衰期识别 + Bateman 动力学 + 冷门 $\mathrm{XeCl}_{4}$ Mossbauer 化学，几乎把"难度顶满"的所有套路叠在一起。对于未读过 Appelman/Perlow 原文的选手，即便给出全部定量数据也几乎无路可寻；对于读过原文的选手则是一道"爽题"。典型的**知识型难题**而非**能力型难题**——与题 1、题 2 的"纯推理+冷门物种"风格略有不同，本题更多依赖背景阅读的厚度。
