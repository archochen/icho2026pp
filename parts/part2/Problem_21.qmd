...
>
> **🟡 TBC — depends on Q21.4.** The three cation *identities* below are fixed by the m/z values alone and are not in dispute: $\mathrm{C_3H_5^+}$, $\mathrm{C_7H_7^+}$, and $\mathrm{C_9H_{11}^+}$. What *is* tentative is the **mechanistic narrative** describing how each cation forms — specifically, which radical is lost in the α-cleavage that gives m/z 119, and which propyl cation rearranges to m/z 41. The narrative below assumes the Q21.4 verdict (the n-butyl product); if Q21.4 changes to the isobutyl alternative, every "n-propyl radical" / "n-propyl cation" reference here changes to "iso-propyl", but the cation **drawings remain valid**.
>
...
>
> **🟡 TBC — verify the substrate drawing.** The mass spectrum and the "max one plane of symmetry" clue are satisfied by **both** structures below (A and B); EI-MS *cannot* tell them apart because the lost α-cleavage radical has the same mass (·n-Pr and ·iso-Pr are both 43). The only experimental discriminator is the alkene drawn in the printed scheme: **unbranched** alkene → answer (A) **1-butyl-4-ethylbenzene**; **methyl-branched** alkene `R-CH=C(CH₃)₂` → answer (B) 1-ethyl-4-isobutylbenzene. We've read the scheme as unbranched, so (A) is the current verdict — but a student reading the scheme differently would converge on (B), and that reading would not be unreasonable. **Please cross-check the scheme image against the printed booklet before treating this as final.**
>
