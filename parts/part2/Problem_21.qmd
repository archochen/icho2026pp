...
>
> **Revision note (2026-05-18).** Earlier revisions proposed (i) a diphenyl-alkene (m/z incompatible), (ii) 1-tert-butyl-4-ethylbenzene (rejected because its EI base peak is m/z 147 from $\alpha$-cleavage at the tert-butyl benzylic position), and (iii) 1-ethyl-4-**isobutyl**benzene. Assignment (iii) gave the correct m/z 119 base peak by $\alpha$-cleavage but was **inconsistent with the substrate scheme**: an isobutyl side chain $-\mathrm{CH_2CH(CH_3)_2}$ would require hydrogenation of a *branched* alkene `Ar-CH=C(CH₃)₂`, which the printed scheme does not show. The scheme depicts an unbranched 3-carbon alkene $\mathrm{R{-}CH{=}CH{-}CH_3}$ (or the terminal isomer $\mathrm{R{-}CH_2{-}CH{=}CH_2}$), which reduces to an **n-propyl** side chain. The substrate-consistent product is therefore the *n-butyl* (not isobutyl) isomer:
>
> $$\boxed{\mathrm{4\text{-}Et\text{-}C_6H_4\text{-}CH_2CH_2CH_2CH_3}}$$
> i.e. **1-butyl-4-ethylbenzene** (CAS 7295-04-5).
>
> **Substrate connection.** With $\mathrm{R}=\mathrm{(4\text{-}Et\text{-}C_6H_4)CH_2{-}}$ (the 4-ethylbenzyl group), the substrate $\mathrm{R\text{-}CH=CH\text{-}CH_3}$ is 1-(4-ethylphenyl)-2-butene, and Wilkinson hydrogenation gives 4-ethyl-1-butylbenzene cleanly. The same product is obtained from $\mathrm{R\text{-}CH_2\text{-}CH=CH_2}=4\text{-}(4\text{-ethylphenyl})\text{-}1\text{-}\text{butene}$.
>
> **Formula and mass.** $\mathrm{C_{12}H_{18}}$, $M_r=162.27\,\mathrm{g\,mol^{-1}}$, matching the molecular-ion cluster near $m/z=162$ in the spectrum.
>
> **Symmetry (one mirror plane).** The benzene ring plane is **not** a symmetry plane of the whole molecule, because the H atoms on each sp$^3$ alkyl carbon lie above and below the ring plane (and a terminal methyl can position only one of its three H's in that plane, never all three). The only symmetry plane is the **vertical plane containing the C1–C4 axis and perpendicular to the ring** — this plane contains the extended n-butyl and ethyl backbones (both of which have a local mirror plane through their C–C–C… chains) and maps the molecule onto itself. The perpendicular vertical plane (through C2–C5, swapping the two substituents) is **not** a mirror because the n-butyl and ethyl groups are different. Hence exactly **one** plane of symmetry, satisfying the question's clue.
>
> **EI-MS rationalisation.** $\alpha$-cleavage at the benzylic position of the n-butyl side chain breaks the $\mathrm{C_\alpha\text{-}C_\beta}$ bond and gives the stable 4-ethylbenzyl cation ($\mathrm{C_9H_{11}^+}$, $m/z=119$, base peak) plus an n-propyl radical ($\mathrm{\cdot CH_2CH_2CH_3}$, $\Delta m=43$):
> $$[\mathrm{M}]^{+\bullet}\rightarrow \mathrm{4\text{-}Et\text{-}C_6H_4\text{-}CH_2^+}\;+\;\mathrm{\cdot C_3H_7}.$$
> Secondary loss of ethene from $m/z=119$ delivers tropylium ($\mathrm{C_7H_7^+}$, $m/z=91$); complementary charge retention on the n-propyl side gives $\mathrm{C_3H_7^+}$ which dehydrogenates to $\mathrm{C_3H_5^+}$ ($m/z=41$). The much smaller $m/z=147$ peak in the spectrum corresponds to the rarer competing $\alpha$-cleavage at the ethyl side ($-\mathrm{CH_3}$), giving 4-butylbenzyl cation. Loss of $\mathrm{CH_3}$ is suppressed relative to loss of $\mathrm{C_3H_7}$ because the longer n-propyl radical is more stable, which is why $m/z=119$ is base and $m/z=147$ is barely visible — exactly the pattern observed.
